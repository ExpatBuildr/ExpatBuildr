...
- [ ] **Enable image optimization on Cloudflare.** Add `imageService: 'compile'` to `astro.config.mjs`. Build currently warns "Cloudflare does not support sharp at runtime" — images are unoptimized, directly hurting LCP (Core Web Vitals ranking factor).
