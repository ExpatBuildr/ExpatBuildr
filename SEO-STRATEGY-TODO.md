...
- [ ] **IndexNow — still not stable, on 2nd key as of 2026-07-21.** Timeline: the galaxybuilt.dev-inherited key (`eb8248...`) never worked (wrong property). Generated a fresh key (`72d1028...`) on 2026-07-20 — a 1-URL trust-build submission got a clean 200/202 from both Bing and IndexNow.org. The very next deploy flipped straight to full bulk (~116 URLs) with that same key — it immediately 403'd (`UserForbiddedToAccessSite`) on both endpoints, and Bing Webmaster Tools' own IndexNow dashboard subsequently stopped recognizing that key as verified at all (flashed a submission-history view showing ~63 prior submissions, then reverted to "Get Started" and offered a new key). Rotated to a 3rd-generation key (`d258eca3...`) on 2026-07-21, hosted at `public/d258eca3033e4c45b4f1fda88efbad3d.txt`, and reset `scripts/pingIndexNow.mjs` to `SINGLE_URL_TRUST_BUILD = true`. Also deleted `src/pages/api/indexnow.ts` — a separate, dead, publicly-reachable GET route that still hardcoded the original invalid galaxybuilt.dev key.
  - [ ] **Do not repeat the jump-to-bulk mistake.** Hold single-URL mode across several *real, separately-timed* deploys (actual days apart) before touching `SINGLE_URL_TRUST_BUILD`. Only after multiple deploys come back clean, ramp `BULK_BATCH_SIZE` in small steps (5 → 15 → 30 → all) with real time between each increase — the script already supports a capped, most-recent-first batch via `BULK_BATCH_SIZE`, ready for when it's time to scale up.
  - [ ] Check back in a few days to confirm the new key stays "verified" in Bing Webmaster Tools' IndexNow dashboard (no repeat of the flash-then-revert-to-"Get Started" behavior).
...
- [x] **Consolidate the tag taxonomy.** Done 2026-07-20. Was 502 unique tags across 114 posts (only 11 indexed, rest correctly `noindex`'d but near-zero internal-linking value). Remapped every post to a 35-tag canonical taxonomy (one pillar-anchor tag + themed sub-tags per pillar, e.g. `Geo-Arbitrage`/`Cost of Living`/`Digital Nomad`/`Southeast Asia`, plus cross-cutting tags like `Expat Founder` and `Automation`). Locked it in `src/content/config.ts` via `tags: z.array(z.enum([...35 tags]))` so a build now fails if a new post uses a tag outside the list — sprawl can't silently come back, a tag can only be added by deliberately editing that enum. `INDEXED_TAGS` in `src/pages/blog/tag/[tag].astro` updated to index all 35 (up from 11) since every tag now clusters 1-20 real posts instead of being a one-off long-tail phrase. Long-tail keyword targeting still lives in `primaryKeyword`/`secondaryKeywords` frontmatter (already populated on 112/114 posts), so nothing was lost by tightening tags into a real taxonomy.
- [x] **Fix `RelatedPosts` to link by relevance, not randomly.** Turned out `RelatedPosts.astro` already scored by shared tags + same pillar (not random) — but with 502 near-unique tags, shared-tag overlap was almost always 0, so it degraded to pillar+recency in practice. The tag consolidation above fixes this for free: posts within a pillar now genuinely share 2-3 tags, so the existing scoring logic produces real relevance-based results without any code change needed.
